...
so as not to clutter the argument list for \[extract_student_info()\].
...
\* n_topics: the number of topics \* R: the optimally desired number of
repetitions per topic \* nmin: the minimum number of students per topic,
\* nmax: the maximum number of students per topic, \* rmin: the minimum
number of repetitions per topic, \* rmax: the maximum number of
repetitions per topic.
