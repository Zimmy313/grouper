...
the solver result through \[assign_groups()\] or \[assign_job()\]
...
- solver:
  ...
  Solver to use through \`ompr.roi\`. Must be one of \`"glpk"\`,
  \`"highs"\`, or \`"gurobi"\`.

- dframe:

  The original dataframe used in \[extract_student_info()\]. Required
  for \`assignment = "diversity"\` and \`assignment = "preference"\`.

- params_list:

  The list of parameters from \[extract_params_yaml()\]. Required for
  \`assignment = "preference"\`.

- group_names:

  A character string denoting the self-formed group column in
  \`dframe\`. Required for \`assignment = "diversity"\` and \`assignment
  = "preference"\`.
...
- time_limit, iteration_limit:
  ...
  Optional Gurobi controls. These are applied only when \`solver =
  "gurobi"\`.
