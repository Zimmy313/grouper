...
For each student-course pair, $P_{i,j}$ denotes the user-supplied TA preference
score. Student year of study is denoted by $y_i$. The E-allocation score is
represented by $s_i$ and defaults to $-1,0,1,2$ for Years 1, 2, 3 and 4,
respectively. Users may replace this four-value encoding during extraction.
We also track prior-semester TA and GR workload, denoted by $t_i^{(1)}$ and
$g_i^{(1)}$.
...
When $\phi > 0$, larger values of $s_i$ make E allocation more attractive.
...
The score vector affects only the E objective. Year-1 protection and the TA
fairness group continue to use `student_df$year`. The preference matrix is also
used exactly as supplied, so users can choose their own numeric scoring during
preprocessing rather than using the example `3/2/1/-99` encoding.
