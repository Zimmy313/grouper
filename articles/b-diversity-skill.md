...
a certain number of times across the class. Suppose that the maximum
number of repetitions of a topic is $`r_{max}`$. In total, there are
$`N`$ students in the class.

Suppose that students form their own groups, which they submit through a
survey form. In total there are $`G`$ such self-formed groups; each
student appears in exactly 1 group.
...
1.  Information that can be used to compute dissimilarities between
    ...
    - gender,
    - year-of-study, etc.
2.  Information on the skill level pertinent to the class, or to the
...
\max \quad w_1 \left( \sum_{i=1}^{N-1} \sum_{j=i+1}^N \sum_{t=1}^T 
                      \sum_{r=1}^{r_{max}} z_{ijtr} \cdot d_{ij} \right) + 
           w_2 (s_{min} - s_{max})
```
...
``` math
\sum_{t=1}^T \sum_{r=1}^{r_{max}} x_{gtr} = 1, \quad \forall g
```
...
\sum_{r=1}^{r_{max}} a_{tr} &\ge& r_{min},\quad \forall t \\
\sum_{r=1}^{r_{max}} a_{tr} &\le& r_{max},\quad \forall t
\end{eqnarray}
```

Take note that the last constraint is automatically satisfied since
{a\_{tr}} is binary.

### Number of students per group

A similar set of constraints are used to bound the number of students in
each eventually assigned group. $`n_{min}`$ and $`n_{max}`$ are also
input variables; they dictate the bounds on the number of students in
each assigned group.

``` math
\begin{eqnarray}
\sum_{i=1}^N \sum_{g=1}^G m_{ig} \cdot x_{gtr} &\ge& a_{tr} \cdot n_{min}, \quad \forall t,r \\
\sum_{i=1}^N \sum_{g=1}^G m_{ig} \cdot x_{gtr} &\le& a_{tr} \cdot n_{max}, \quad \forall t,r 
\end{eqnarray}
```

### Per-group skill levels

We aim to maintain the skill level within each active group using the
following constraints. Let $`M=\sum_{i=1}^N s_i`$.

``` math
\begin{eqnarray}
\sum_{i=1}^N \sum_{g=1}^G m_{ig} \cdot x_{gtr} \cdot s_i + M(1-a_{tr}) &\ge& s_{min}, \quad \forall t,r \\
\sum_{i=1}^N \sum_{g=1}^G m_{ig} \cdot x_{gtr} \cdot s_i &\le& s_{max}, \quad \forall t,r \\
\end{eqnarray}
```

When $`a_{tr}=1`$, the lower bound applies to the total skill in that
active topic-repetition combination. When $`a_{tr}=0`$, the Big-M term
relaxes the lower bound so inactive combinations do not force
$`s_{min}`$ to zero.
