...
constraints. Doctoral teaching-workload allocation motivated the design:
individuals may receive teaching assistantship (TA), grading (GR), and lighter
residual support duties (E). The resulting formulation is not specific to that
setting. TA and GR can each have their own preference, fairness, and
protected-cohort policies, while E can be allocated according to a score such as
seniority, experience, or eligibility.

The extractor aligns an individual table, optional role-specific preference
matrices, and a role-demand matrix. The individual table supplies cohort year
...
The first two terms penalize annual TA and GR workload spread. The next two
reward role-specific preference fit, the E term rewards allocation according to
the supplied score, and the final terms penalize violations of the two
protected-cohort soft limits. Each objective component is optional, so the
formulation can represent policies that use only a subset of these criteria.
...
\[
\left(t_i^{(1)}+\sum_jX_{ij,\mathrm{TA}}\right)
+\left(g_i^{(1)}+\sum_jX_{ij,\mathrm{GR}}\right)
...
T_{\min},T_{\max},G_{\min},G_{\max}\geq0,\qquad
w_i^{\mathrm{TA}},w_i^{\mathrm{GR}}\geq0. \tag{16}
\]
...
Setting an objective weight to zero or \texttt{NULL} disables that policy component.
Preference and E expressions are omitted from the objective; disabling a
fairness or protection term also omits its supporting variables and
constraints. When a protection term is disabled, all individuals enter that
role's fairness calculation if the corresponding fairness term remains active.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{alpha\_ta}, \texttt{alpha\_gr} & increases the penalty on annual workload spread for the corresponding role & can improve role-specific fairness at the expense of preference, priority, or protection terms \\
\texttt{beta\_ta}, \texttt{beta\_gr} & increases the reward for the corresponding role's preference scores & can improve role-specific preference attainment at the expense of other objective terms \\
\texttt{phi} & increases the reward for assigning E to individuals with larger scores & can improve priority alignment at the expense of the other objective terms \\
\texttt{rho\_ta}, \texttt{rho\_gr} & makes the corresponding protected-cohort slack more expensive & discourages overload but does not alter feasibility because slack remains available \\
\texttt{ta\_protected\_max}, \texttt{gr\_protected\_max} & raises the corresponding protected cohort's soft threshold & weakens protection and can reduce the slack penalty; formal feasibility is unchanged \\
...
workflow, \texttt{assignment\ =\ "multirole"} accepts the individual table,
role-demand matrix, optional TA and GR preference matrices, semester capacity,
and history mode. In all cases, the purpose of \texttt{extract\_info()} is to align
...
evaluates that workflow using institutional planning data from AY2420, AY2510,
and AY2520. The notation
...
The generalized model permits independent GR fairness, preference, and
protected-cohort terms. The institutional comparison retains the policy used in
the original planning exercise: TA fairness, TA preferences, seniority-guided E
allocation, and Year-1 TA protection are active, while the GR-specific objective
terms are disabled. In particular, no GR preference term is evaluated because
the retained manual records do not identify GR assignments by course. The
comparison therefore demonstrates a TA-focused configuration of the general
multi-role model rather than every available policy component.
...
included as the third role column, so \texttt{e\_mode} is set to \texttt{"none"}.
...
vectors; downstream code no longer needs to know the original file layout.
...
\texttt{ta\_protected\_max\ =\ 1} limits Year-1 teaching assistantship load before slack is
...
TA workload fairness, TA preference fit, seniority steering, and Year-1 TA
protection under the stated policy weights. GR demand and total workload remain
feasibility requirements, but no GR-specific objective term contributes to this
comparison.
...
this sense, \texttt{grouper} makes allocation rules explicit and searches the feasible
...
tuning constants. The disabled GR weights are not varied because they are
outside the institutional policy evaluated here.
...
runtime of 0.181 seconds and an observed
range from 0.174 to
0.241 seconds. \texttt{HiGHS} also returned
objective value \(-182\) in all 30
runs, with a mean runtime of 0.195
seconds and an observed range from 0.189
to 0.243 seconds. These timings indicate
...
attainment, and the wide assignment table produced by \texttt{assign\_job()}.
