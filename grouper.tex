...
Each \texttt{grouper} workflow is formulated as an integer program.
...
policy constraints. The extraction step receives three order-aligned input
objects:
...
E-demand mode, optional E-allocation scores, and history mode. For multi-role
inputs, row and column order must already be aligned across the individual
table, demand matrix, and any supplied preference matrices. In all cases, the
purpose of \texttt{extract\_info()} is to translate user-supplied records into the
mathematical objects used in the optimization model.

The following illustrative call uses \(S\), \(P_{\mathrm{TA}}\),
\(P_{\mathrm{GR}}\), and \(D\) to denote the individual table, two preference
matrices, and demand matrix.

\begin{verbatim}
# "diversity": dframe + self_formed_groups + d_mat/demographic_cols
# "preference": dframe + self_formed_groups + pref_mat
# "multirole": order-aligned S, D, and optional role-specific preferences
df <- extract_info(
  "multirole",
  student_df = S,
  d_mat = D,
...
For multi-role output, the task-code vector \(J\) is supplied at this stage as
\texttt{course\_codes}, and its order must match the row order of the demand matrix used
during extraction.

\begin{verbatim}
# solver can be "glpk", "highs", or "gurobi" when available
# group workflows pass dframe, params_list, and group_names
# multi-role passes student_df and course_codes (J) for post-processing
...
\item
  TA preference fit
\item
  seniority-guided E allocation
\item
  Year-1 TA protection
\end{enumerate}
...
The multi-role workload tab validates the current-semester template and, unless
single-semester mode is selected, a previous \texttt{assign\_job()} output. It
standardizes names and course codes, pads prior workload to the semester
capacity when needed, and supports synthetic history in single-semester mode.
The tab constructs the workload input with \texttt{extract\_multirole\_info()}, builds the
optimization model with \texttt{prepare\_multirole\_model()}, solves it through an
ROI-backed \texttt{ompr} call, and formats the wide output table with \texttt{assign\_job()}.
Its controls expose independent TA and GR weights, protection settings, role
bounds, E seniority scores, solver controls, and single-semester mode. Direct R
users can still access the full lower-level API, including separate TA and GR
preference matrices and other extraction choices. After a successful run, the
app displays solver status, objective diagnostics, a workload-distribution plot,
preference attainment, and the assignment table.

DBA and PBA outputs can be downloaded as CSV files, while the multi-role
assignment table can be downloaded as an XLSX file for further processing or
record-keeping.
...
\caption{Multi-role workload run summary and workload distribution view.}\label{fig:fig-grouper-webapp-multirole-results}
\end{figure}
