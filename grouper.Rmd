...
operationalize. Manual allocation can be time-consuming, difficult to
reproduce, and hard to audit when multiple constraints and policy goals
interact.

The \pkg{grouper} package provides a high-level R workflow for
...
The contribution of \pkg{grouper} is both mathematical and computational. Mathematically,
...
workflows of \pkg{grouper}. The next section reviews related work, including
...
\pkg{grouper} differs from these approaches by providing a higher-level package
...
Each \pkg{grouper} workflow is formulated as an integer or mixed-integer program.
...
Consider \(N\) students arranged into \(G\) self-formed groups. Each group is
assigned to a topic \(t\) from \(T\) base topics, where $T$ lies between
$r_{min}$ and $r_{max}$. A topic may contain \(B\) subgroups, giving \(BT\)
topic-subgroup combinations.  This setting arises in the allocation of
pre-determined project topics, case studies, supervisors, lab rotations,
presentation themes, and tutorial or project streams. The concept of sub-topics
is specific to our use-case (see Section \@ref(sec-pba-real-life), but it is
not necessary. By setting $B=1$, it is possible to use this \pkg{grouper} model
...
\@ref(tab:pba-param-guide) summarises the inputs for the diversity-based allocation
model.
...
constraints. The problem formulation is motivated by PhD workload allocation,
where students receive teaching assistantship, grading, and residual support
duties.  Formally, let role \(A\) be a preference-sensitive one, role \(B\) be
routine or demand-driven, and role \(C\) be allocated according to a priority
score such as seniority, experience, or eligibility. In the problem genesis,
role $A$ corresponded to teaching duties, role $B$ to grading, and role $C$ to
minor miscellaneous duties over a semester.

The extractor aligns a student table, preference matrix, and role-demand
matrix.  The student table supplies cohort year and previous teaching and
grading workload, which become the priority score \(q_i\) and historical
workload vectors \(t_i^{(1)}\) and \(g_i^{(1)}\). The preference matrix
supplies \(P_{ij}\), which corresponds to the preference that student $i$ has
for task $j$, typically only for role $A$. The demand matrix supplies
\(d_{jr}\), which corresponds to the demand for role $r$ in task $j$.
Residual-role demand may either be provided or derived from total semester
workload and submitted teaching and grading demand.
...
constraints. Please refer to Table \@ref(tab:multirole-param-guide) for a
summary of inputs.
...
knitr::kable(multirole_param_guide, booktabs = TRUE,
             col.names = c("Parameter", "If increased", "Trade-off"),
...
# The \pkg{grouper} package {#sec-grouper-pkg-wrapper}

Figure \@ref(fig:fig-package-architecture) summarizes the architecture of
\pkg{grouper} as a domain-specific implementation layer for the allocation models
...
In this section, we use the historical `assignment = "phd"`
...
In the following call, `S`, `P`, `D`, and `J` denote the student table,
preference matrix, demand matrix, and course-code vector.
...
software contribution of \pkg{grouper} relative to lower-level modeling toolkits
such as `ompr`, `ROI`, and solver interfaces. Those tools provide flexible
optimization infrastructure, whereas \pkg{grouper} provides a domain-specific layer
...
In order to understand how \pkg{grouper} works, we demonstrate it's usage on
...
`id` column is strictly not necessary for \pkg{grouper} to work, it is good
to include it.
...
mapped to the original dataset, \pkg{grouper} provides a convenience function
`assign_groups()` to perform the required table join.
...
The final output dataset, displayed in Table \@ref(tab:constructed-dba-2), is
identical to the original data `dba_gc_ex001` except for the columns `topic` and
`rep`. The allocation agrees with the stated intuition that students 1 and 4
...
\pkg{grouper}. In all three scenarios, we revisit semesters where allocation
had been carried out manually, and consider how helpful it would have been, to
have \pkg{grouper} available.
...
the manual, and retrospective \pkg{grouper} output, for one such tutorial class
...
it was necessary to divide students into project teams with two sub-groups: one
sub-group typically developed a user interface, and the other worked on data
analysis. Each project team then needed to be assigned to one of a set of
pre-determined project titles. As there were more titles than project teams,
there was a need to assign multiple teams to the same title; hence the need for
repetitions. 

Table \@ref(tab:pba-past-1) shows how \pkg{grouper} could have benefited
...
\@ref(tab:pba-past-1)): the enrolment was 62 and there were 4 topic titles to
be allocated. In the manual allocation carried out, the average preference of
the self-formed groups was 8.16. *If we could have used* \pkg{grouper}, the average
...
Figure \@ref(fig:pba-past-2) confirms that in every semester, \pkg{grouper}
...
sense, \pkg{grouper} makes allocation rules explicit and searches the feasible
...
\pkg{grouper} also ships with a \CRANpkg{shiny} [@R-shiny] frontend for users who prefer a
browser-based workflow over direct R scripting. The unified application is stored
under `inst/shiny/grouperWebApp/` and is built with \CRANpkg{bslib} [@R-bslib] and
interactive tables from \CRANpkg{DT} [@R-DT]. It is intended for instructors and
...
without writing the wrapper calls by hand. The landing page of the application
is shown in Figure \@ref(fig:fig-grouper-webapp-home).
...
\pkg{grouper} is designed for higher-education allocation settings where policy
...
custom solver in \pkg{grouper}, but solver availability, installation, and licensing
...
only has to consider pre-processing steps (for `extract_info()` to call), and
a model formulation using `ompr` (for `prepare_model()` to use).

# Conclusion

\pkg{grouper} provides three mathematical models and a consistent R workflow for
...
Overall, \pkg{grouper} fills a practical gap between low-level optimization toolkits
