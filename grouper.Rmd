...
abstract: >
  ...
  groups, preference-based assignment of students to topics,
  and multi-role workload allocation under fairness, preference, and
  ...
  structured input formats, configurable constraints and objective weights. It
  offers consistent entry and exit points to and from the model.
  Users can adapt group-size
...
[@anwar2003student; @meyer2009optassign]. On CRAN, \CRANpkg{golfr} provides classroom
group assignment functionality with a focus on minimizing repeated pairings
across multiple rounds [@R-golfr]. These tools address important sub-problems,
...
# Mathematical allocation models {#sec-math-models}
...
```{r multirole-param-guide, echo=FALSE}
...
```
# The \CRANpkg{grouper} package {#sec-grouper-pkg-wrapper}
...
```{r fig-package-architecture, fig.cap="High-level package architecture.", fig.alt="Workflow diagram showing higher-education inputs flowing through data processing, model building, and solving stages, with outputs for solver diagnostics and allocation tables.", out.width="0.92\\linewidth", echo=FALSE, fig.pos='ht'}
...
In this section, we use the `assignment = "multirole"`
...
The `extract_info()` function standardizes raw inputs into a list structure
...
workflow, `assignment = "multirole"` accepts the individual table, role-demand
...
the purpose of `extract_info()` is to translate user-supplied records into the
mathematical objects used in the optimization model.

The following illustrative call uses `S`, `P_TA`, `P_GR`, and `D` to denote
the individual table, TA preference matrix, optional GR preference matrix, and
role-demand matrix.


```{r wrapper-extract-info, eval=FALSE}
...
```{r wrapper-prepare-model, eval=FALSE}
...
course-code vector `J` is passed as `course_codes`, and its order must match the
row order of the demand matrix used during extraction.

```{r wrapper-solve-assignment, eval=FALSE}
...
# Constructed datasets {#sec-constructed-data}
...
The dataset for this example `dba_gc_ex001` is available with the package.  The
...
`id` column is strictly not necessary for \CRANpkg{grouper} to work, it is good
to include it.

```{r constructed-dba-data}
dba_gc_ex001
```
...
`dba_ex1_result$output`.  

```{r constructed-dba-2, echo=FALSE}
...
`grouping` column. Here is a listing of the dataset:

```{r constructed-pba-data}
pba_gc_ex002
```
...
```{r constructed-pba-2, echo=FALSE}
...
# Examples from past semesters {#sec-semester-data}
...
the self-formed groups was 8.16. *If we could have used* \CRANpkg{grouper}, the average
preference for this same group of students would have been 9.47 (higher is
better).

```{r pba-past-1, echo=FALSE}
...
`ta_protected_max = 1` limits Year-1 TA load before slack is
...
```{r load-solver-runtime, include=FALSE}
...
\CRANpkg{grouper} also ships with a \CRANpkg{shiny} [@R-shiny] front end for users
who prefer a guided browser workflow to direct R scripting. The unified
application is stored under `inst/shiny/grouperWebApp/` and is built with
\CRANpkg{bslib} [@R-bslib] and interactive tables from \CRANpkg{DT} [@R-DT]. It
...
```{r fig-grouper-webapp-home, fig.cap="Home page of the web application. The page acts as a model selector, routing users to the DBA, PBA, or multi-role workload workflow.", fig.alt="Screenshot of the grouper Shiny home page with three model cards.", out.width="0.86\\linewidth", fig.pos="!htbp", echo=FALSE}
...
implementation, so solver availability still depends on installed `ROI` plugins
and `Gurobi`-specific controls only apply with the `Gurobi` backend. For
...
only has to consider preprocessing steps (for `extract_info()` to call), and
a model formulation using `ompr` (for `prepare_model()` to use).
