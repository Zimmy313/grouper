...
Each `grouper` workflow is formulated as an integer program.
...
purpose of `extract_info()` is to translate user-supplied records into the
...
`course_codes`, and its order must match the row order of the demand matrix used
during extraction.
...
single-semester mode is selected, a previous `assign_job()` output. It
...
The tab constructs the workload input with `extract_multirole_info()`, builds the
optimization model with `prepare_multirole_model()`, solves it through an
ROI-backed `ompr` call, and formats the wide output table with `assign_job()`.
