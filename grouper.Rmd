...
In order to understand how \pkg{grouper} works, we demonstrate it's usage on two
small and artificially constructed datasets in this section. This allows new users 
to understand which arguments are important, and the role that they play.

## Diversity-based assignment

The dataset for this example `dba_gc_ex001` is available once the package is
loaded. While the `id` column is strictly not necessary, it is good to include
it. Inspection of the data should show that, if we needed to create two groups
of students based on maximising the diversity of major only, then one group
should comprise students 1 and 3, and the other should comprise students 2 and
4.
...
dataset, using the functions described in \@ref(sec-grouper-pkg-wrapper).
...
`asssign_groups` to perform the required table join.
...
In this case, it is possible to assign each self-formed group to its optimal
choice of topic-subtopic combination. In our solution, we should see that group
1 is assigned to subtopic 1 of topic 1, group 2 is assigned to sub-topic 1 of
topic 2, and so on.
...
the original dataset. The allocation is shown in \@ref(tab:constructed-pba-2).
